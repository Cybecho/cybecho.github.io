...
description: "ProxMox의 네트워크 모드는 브리지, 라우팅, NAT, 본딩으로 나뉘며, 각 방식은 가상 머신의 인터넷 연결 방식에 따라 다릅니다. 브리지는 가상 스위치를 통해 직접 연결을 제공하고, 라우팅은 서버가 모든 네트워크 신호를 처리합니다. NAT는 내부 IP를 사용하여 외부 인터넷에 접속하며, 본딩은 여러 네트워크 포트를 결합하여 안정성과 속도를 향상시킵니다."
...
이럴 때는 두 번째 방식인 '라우팅'을 사용합니다. 이 방식에서는 프록스목스 서버가 중간에서 교통정리를 해줘요. 

가상 컴퓨터들의 모든 네트워크 연결을 서버가 받아서 외부로 전달하는 방식이죠. 이렇게 하면 하나의 연결로도 여러 가상 컴퓨터를 인터넷에 연결할 수 있습니다.
...
서버에는 보통 인터넷 선을 꽂는 포트가 여러 개 있어요. 두 개의 포트가 있다고 가정해볼까요?

기본적으로는 한 포트만 사용하게 되는데, 본딩을 사용하면 두 포트를 하나로 합칠 수 있어요. 이렇게 하면 두 가지 좋은 점이 있습니다:

1. 안정성이 높아져요: 만약 한 포트에 문제가 생겨도 다른 포트가 계속 작동하기 때문에 인터넷이 끊기지 않아요.
2. 속도가 빨라질 수 있어요: 네트워크 장비가 지원한다면, 두 포트를 동시에 써서 더 빠른 속도를 낼 수 있어요.

쉽게 비유하자면, 본딩은 마치 두 개의 길을 하나로 합쳐서 더 넓고 안전한 길을 만드는 것과 같아요. 차가 한 길에서 막혀도 다른 길로 다닐 수 있고, 두 길을 모두 사용하면 더 많은 차가 다닐 수 있죠.
