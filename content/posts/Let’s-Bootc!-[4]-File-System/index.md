...
description: "bootc 시스템의 파일시스템은 세 가지 영역으로 나뉘며, 각각의 특성이 다릅니다. /usr는 완전히 불변이며, 패키지 설치는 이미지 빌드 시에만 가능하고, /etc는 수정 가능하지만 3-way merge가 적용되어 로컬 수정과 이미지 변경이 충돌할 수 있습니다. /var는 완전히 가변이며, 애플리케이션 데이터와 상태를 저장하는 곳으로 이미지 업데이트의 영향을 받지 않습니다. 이러한 구조는 불변 인프라 원칙을 구현한 결과입니다."
...
> **Summary**
> bootc 시스템의 파일시스템은 세 가지 영역으로 나뉘며, 각각의 특성이 다릅니다. /usr는 완전히 불변이며, 패키지 설치는 이미지 빌드 시에만 가능하고, /etc는 수정 가능하지만 3-way merge가 적용되어 로컬 수정과 이미지 변경이 충돌할 수 있습니다. /var는 완전히 가변이며, 애플리케이션 데이터와 상태를 저장하는 곳으로 이미지 업데이트의 영향을 받지 않습니다. 이러한 구조는 불변 인프라 원칙을 구현한 결과입니다.

---

![Image](image_c0af5b322d0f.png)

![Image](image_d2dbe040228a.png)
