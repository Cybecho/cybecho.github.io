...
| 변경 유형 | 영향 | 예시 |
| 베이스 이미지 변경 | 모든 레이어 재빌드, 빌드 시간 증가 | `FROM ubuntu:22.04` → `FROM alpine:3.14` |
| RUN 명령어 수정 | 해당 레이어 및 이후 레이어 재빌드 | `apt-get install nginx` 추가 |
| COPY 파일 변경 | 해당 레이어 및 이후 레이어 재빌드 | 소스 코드 파일 수정 |
| 파일 위치 변경 | 컨테이너 파일 시스템 구조 변경, 애플리케이션 오류 가능 | 파일 디렉토리 이동 |
