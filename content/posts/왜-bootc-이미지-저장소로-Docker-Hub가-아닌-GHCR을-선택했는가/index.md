...
draft: false
tags: ["Docker"]
series: ["Infra & Network", "Let's Homelab!", "Let's Linux!"]
description: "bootc는 컨테이너 이미지를 OS로 직접 부팅하는 기술로, Docker Hub의 한계로 인해 GHCR로 마이그레이션을 결정했다. Docker Hub는 대용량 이미지 전송에 적합하지 않으며, GHCR은 무제한 저장 용량과 안정적인 대용량 전송을 제공한다. GHCR은 GitHub 생태계와 통합되어 있어 bootc와 같은 대용량 이미지에 적합하다."
...
> **Summary**
> bootc는 컨테이너 이미지를 OS로 직접 부팅하는 기술로, Docker Hub의 한계로 인해 GHCR로 마이그레이션을 결정했다. Docker Hub는 대용량 이미지 전송에 적합하지 않으며, GHCR은 무제한 저장 용량과 안정적인 대용량 전송을 제공한다. GHCR은 GitHub 생태계와 통합되어 있어 bootc와 같은 대용량 이미지에 적합하다.

---

![Image](image_43114f414bad.png)
