...
series: ["Let's MAC!"]
description: "MAC 터미널에서 crontab의 기본 에디터를 vim에서 nano로 변경하려면 export VISUAL=nano; crontab -e 명령어를 사용하고, 다시 vim으로 변경하려면 export VISUAL=vim; crontab -e를 입력하면 된다. 또한, MAC에서 파이썬 코드를 스케줄링할 때는 /usr/bin/python3 /Users/your/path/python_script.py 형식으로 경로를 지정해야 하며, 현재 실행 중인 파이썬 코드는 ps aux | grep python3 명령어로 확인할 수 있다."
...
> **Summary**
> MAC 터미널에서 crontab의 기본 에디터를 vim에서 nano로 변경하려면 export VISUAL=nano; crontab -e 명령어를 사용하고, 다시 vim으로 변경하려면 export VISUAL=vim; crontab -e를 입력하면 된다. 또한, MAC에서 파이썬 코드를 스케줄링할 때는 /usr/bin/python3 /Users/your/path/python_script.py 형식으로 경로를 지정해야 하며, 현재 실행 중인 파이썬 코드는 ps aux | grep python3 명령어로 확인할 수 있다.
